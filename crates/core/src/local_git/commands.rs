...
pub(super) fn git_stdout<const SIZE: usize>(path: &Path, args: [&str; SIZE]) -> VcsResult<String> {
...
pub(super) fn run_git<const SIZE: usize>(path: &Path, args: [&str; SIZE]) -> VcsResult<Output> {
    let mut command = Command::new("git");
    command.current_dir(path).args(args);
...
fn command_output(mut command: Command) -> VcsResult<Output> {
    let output = command
        .output()
        .map_err(|io_error| error().invalid_input(io_error.to_string()))?;
